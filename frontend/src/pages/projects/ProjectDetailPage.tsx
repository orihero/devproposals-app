...
const ProjectDetailPage: React.FC = () => {
  ...
  const [hasProjectSummary, setHasProjectSummary] = useState(false);
    ...
    const fetchProjectDetails = async () => {
      ...
      try {
        ...
        try {
          ...
          setHasProjectSummary(!!summary);
        } catch (error: any) {
          // If summary not found, set to false
          if (error.message === 'Resource not found' || error.message === 'Project summary not found') {
            setHasProjectSummary(false);
          } else {
            console.error('Error checking project summary:', error);
            setHasProjectSummary(false);
          }
        }
      } catch (err: any) {
  ...
  const generateComparisonSummary = async () => {
    ...
    try {
      ...
      setShowComparisonSummary(true);
              ...
              // Create a new ProjectSummary in the database
        try {
          // Calculate average cost and duration from proposals
          const validProposals = proposals.filter(p => p.totalCost && p.timeline);
          const avgCost = validProposals.length > 0 
            ? Math.round(validProposals.reduce((sum, p) => sum + (p.totalCost || 0), 0) / validProposals.length)
            : 0;
          const avgDuration = validProposals.length > 0
            ? Math.round(validProposals.reduce((sum, p) => sum + (p.timeline || 0), 0) / validProposals.length)
            : 0;
          
          const projectSummaryData: CreateProjectSummaryRequest = {
            summary: response.summary.summaryContent,
            keyPoints: [
              `Generated comparison summary for ${response.summary.totalProposals} proposals`,
              `Project: ${response.summary.projectTitle}`,
              `Analysis completed on ${new Date(response.summary.generatedAt).toLocaleDateString()}`,
              `Average estimated cost: $${avgCost.toLocaleString()}`,
              `Average estimated duration: ${avgDuration} days`
            ],
            recommendations: [
              'Review the comparison summary for decision making',
              'Consider the analysis scores when evaluating proposals',
              'Use the insights to guide project direction',
              'Compare the average estimates with your budget and timeline requirements'
            ],
            technicalDetails: `This summary was generated by AI analysis of ${response.summary.totalProposals} proposals for the project "${response.summary.projectTitle}". The analysis includes comparison scores, key insights, and recommendations based on the submitted proposals. Average cost estimate: $${avgCost.toLocaleString()}, Average duration estimate: ${avgDuration} days.`,
            estimatedCost: avgCost,
            estimatedDuration: avgDuration,
            complexity: avgCost > 50000 || avgDuration > 90 ? 'high' : avgCost > 25000 || avgDuration > 45 ? 'medium' : 'low'
          };
          
          const savedSummary = await projectSummaryService.createOrUpdateProjectSummary(projectId, projectSummaryData);
          console.log('✅ Project summary created successfully:', savedSummary);
          
          // Update the hasProjectSummary state
          setHasProjectSummary(true);
          
          // Trigger a complete refetch of all project data
          setRefreshTrigger(prev => prev + 1);
          
          // Show success message to user
          alert('Comparison summary generated and project summary saved successfully!');
        } catch (summaryError: any) {
          console.error('Failed to save project summary:', summaryError);
          alert('Comparison summary generated, but failed to save project summary. Please try again.');
        }
    } catch (err: any) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <ProjectSummary 
          ...
          onSummaryGenerated={(summary) => {
            console.log('Project summary loaded:', summary);
            setHasProjectSummary(!!summary);
          }}
          onRegenerate={generateComparisonSummary}
          onSummaryUpdated={() => {
            // Trigger a complete refetch of all project data when summary is updated
            setRefreshTrigger(prev => prev + 1);
          }}
        />
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                {proposals.length > 1 && !hasProjectSummary && (
                  <button
